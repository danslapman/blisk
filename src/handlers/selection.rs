...
use crate::parsing::scala::{node_to_range, position_to_point};
...
fn selection_range_at(tree: &Tree, pos: Position) -> SelectionRange {
    ...
    // Find the innermost named node containing the position
    let mut node = root
    ...
    // Build SelectionRange linked list from outermost (None parent) to innermost.
    // LSP: `parent.range` must CONTAIN `this.range`, so parent = outer scope.
